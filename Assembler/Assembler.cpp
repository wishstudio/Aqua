...
std::string compile_method(int *register_count, int *code_size, int *exception_clause_count)
{
	...
	std::function<void()> compile_method_internal = [&]() {
		...
		for (;;)
		{
			...
			OP("jeqi", BINADDR(0x76))  OP("jequ", BINADDR(0x76))
			OP("jneqi", BINADDR(0x77)) OP("jnequ", BINADDR(0x77))
			...
			OP("ldfi", LDF(0x84, 0x8C)) OP("ldfu", LDF(0x84, 0x8C))
			OP("ldfl", LDF(0x85, 0x8D)) OP("ldful", LDF(0x85, 0x8D))
			...
			OP("ldsi", LDS(0x94, 0x9C)) OP("ldsu", LDS(0x94, 0x9C))
			OP("ldsl", LDS(0x95, 0x9D)) OP("ldsul", LDS(0x95, 0x9D))
			...
			OP("ldii", LDI(0xA4, 0xAC)) OP("ldiu", LDI(0xA4, 0xAC))
			OP("ldil", LDI(0xA5, 0xAD)) OP("ldiul", LDI(0xA5, 0xAD))
			...
			OP("ldei", LDE(0xB4, 0xBC)) OP("ldeu", LDE(0xB4, 0xBC))
			OP("ldel", LDE(0xB5, 0xBD)) OP("ldeul", LDE(0xB5, 0xBD))
